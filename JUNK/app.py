...
def convert_spt():
    if request.method == 'POST':
        ...
        if not uploaded_files or not output_folder:
            return render_template('convert_spt.html', message="Please select files and enter an output folder path.")
        ...
        try:
            print(f"Converting {len(saved_files)} files to NetCDF...")
            convert_spt_to_nc(temp_input_folder, output_folder)  # Ensure function works

            # Check if NetCDF files exist
            converted_files = [f for f in os.listdir(output_folder) if f.endswith(".nc")]
            if not converted_files:
                return render_template('convert_spt.html', message="No NetCDF files were created. Please check logs.")

            # Create ZIP file
            zip_filename = os.path.join(output_folder, "converted_files.zip")
            with zipfile.ZipFile(zip_filename, 'w') as zipf:
                for file in converted_files:
                    file_path = os.path.join(output_folder, file)
                    zipf.write(file_path, file)

            return render_template('convert_spt.html', message=f"Conversion completed! Download <a href='/download_zip?path={zip_filename}'>converted_files.zip</a>")
        
        except Exception as e:
            return render_template('convert_spt.html', message=f"Error: {e}")
